...
def check_select_category(info):
    categories = []


    risk_dict = pickle.load(open('./data/risk_dict', 'rb'))
    #type of risk can be determined from category
    
    for key in risk_dict.keys():
        if 'category' in info:
            idx = info.index('category')
            if info[idx+1] in risk_dict[key] or info[idx+2] in risk_dict[key] or info[idx+3] in risk_dict[key] or info[idx+4] in risk_dict[key] or info[idx+5] in risk_dict[key] or info[idx+6] in risk_dict[key]:
                categories.append(key)
        else:
            for i in risk_dict[key]:
                if i in info:
                    categories.append(key)
                    break
                ...
                for docx_file in docx_files:
                    ...
                    if docx_file.type == "text/plain":
                        ...
                        try:
                            ...
                            #categories = select_category(preprocessed_text)
                            categories = check_select_category(preprocessed_text)
                            st.write(categories)
