...
def update_scores(df, info, country, month):
    ...
    if 'importance' in info:
        ...
        if info[idx+1] == 'high':
            val = df.loc[country, month]['importance']
            df.loc[country, month]['importance'] = val + 0.5

    if 'level' in info:
        idx = info.index('level')
        if info[idx+1] == 'notice':
            val = df.loc[country, month]['level']
            df.loc[country, month]['level'] = val + 0.5

        elif info[idx+1] == 'advisory':
            val = df.loc[country, month]['level']
            df.loc[country, month]['level'] = val + 1
    ...
    if 'severity' in info:
        ...
        val = df.loc[country, month]['severity']

        if info[idx+1] == 'score':
            try:
                df.loc[country, month]['severity'] = val + w2n.word_to_num(info[idx+2])
            ...
            if info[idx+1] == 'minor':
                df.loc[country, month]['severity'] = val + 0.5

            elif info[idx + 1] == 'moderate':
                df.loc[country, month]['severity'] = val + 1

            elif info[idx + 1] == 'severe':
                df.loc[country, month]['severity'] = val + 2
...
def update_risks(df, risks, country, month):
    ...
    for risk in risks:
        val = df.loc[country, month][risk]
        df.loc[country, month][risk] = val + 1
